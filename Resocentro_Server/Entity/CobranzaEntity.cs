...
namespace Resocentro_Server.Entity
{
    class CobranzaEntity
    {
    }
    ...
    public class DetalleDocumentoSunat
    {
       int cantidad_decimales=5;
        ...
        public double valorReferencial
        {
            get
            {
                return Math.Round((valorReferencialIGV / (1 + valorIgvActual)), cantidad_decimales, MidpointRounding.AwayFromZero);
            }
        }
        ...
        public double valorUnitario
        {
            get
            {
                return Math.Round((valorUnitarioigv / (1 + valorIgvActual)), cantidad_decimales, MidpointRounding.AwayFromZero);
            }
            set
            {
                _valorUnitarioigv = value;
        ...
        public double descPromocion
        {
            get
            {
                ...
                */
                return Math.Round(((valorUnitario) * (porcentajeDescPromocion / 100)), cantidad_decimales, MidpointRounding.AwayFromZero);
            }
        }
        /// <summary>
        ...
        public double descxCobPaciente
        {
            get
            {
                ...
                {
                    double valor = Math.Round(((valorUnitario - descPromocion) * (porcentajeCobPaciente / 100)), cantidad_decimales, MidpointRounding.AwayFromZero);

                    if (valor >= (valor - montoMaximoAseguradora))
                        return valor;
                    else
                        return Math.Round(((valorUnitario - descPromocion) - montoMaximoAseguradora), cantidad_decimales, MidpointRounding.AwayFromZero);
                }
        ...
        public double descxCobSeguro
        {
            get
            {
                ...
                {
                    double valor = Math.Round(((valorUnitario - descPromocion) * (porcentajeCobSeguro / 100)), cantidad_decimales, MidpointRounding.AwayFromZero);

                    if (valor <= montoMaximoAseguradora)
                        return valor;
                    else
                        return Math.Round((montoMaximoAseguradora), cantidad_decimales, MidpointRounding.AwayFromZero);
                }
            }
        }
        ...
        public double descxItem
        {
            get
            {
                if (isCortesia)
                    if (tipo_cobranza == (int)TIPO_COBRANZA.PACIENTE)
                    { return Math.Round(((valorUnitario - descxCobSeguro - descPromocion) * (porcentajedescxItem / 100)), cantidad_decimales, MidpointRounding.AwayFromZero); }
                    else
                    { return Math.Round(((valorUnitario - descxCobPaciente - descPromocion) * (porcentajedescxItem / 100)), cantidad_decimales, MidpointRounding.AwayFromZero); }
                else
                    return 0;
            }
        }
            ...
            get
            {
                if (tipo_cobranza == (int)TIPO_COBRANZA.PACIENTE)
                {
                    if (isGratuita)
                        ...
                        return Math.Round(((valorUnitario - descxCobSeguro - descxItem - descPromocion) * cantidad), cantidad_decimales, MidpointRounding.AwayFromZero);
                }
                else
                { return Math.Round(((valorUnitario - descxCobPaciente - descxItem - descPromocion) * cantidad), cantidad_decimales, MidpointRounding.AwayFromZero); }
        ...
        public double valorReferencialVenta
        {
            ...
            {
                if (tipo_cobranza == (int)TIPO_COBRANZA.PACIENTE)
                { return Math.Round(((valorReferencial - descxCobSeguro - descPromocion) * cantidad) * (1 + valorIgvActual), cantidad_decimales, MidpointRounding.AwayFromZero); }
                else
                { return Math.Round(((valorReferencial - descxCobPaciente - descPromocion) * cantidad) * (1 + valorIgvActual), cantidad_decimales, MidpointRounding.AwayFromZero); }
        ...
        public double igvItem_old
        {
            get
            {
                ...
                {
                    return Math.Round((valorVenta * valorIgvActual), cantidad_decimales, MidpointRounding.AwayFromZero);
                }
            }
        }
        ...
        public double valorventaImpresion
        {
            ...
            {
                if (tipo_documento == (int)TIPO_DOCUMENTO_ELECTRONICO.BOLETA_DE_VENTA)
                    return Math.Round(valorVenta * (valorIgvActual + 1), cantidad_decimales, MidpointRounding.AwayFromZero);
