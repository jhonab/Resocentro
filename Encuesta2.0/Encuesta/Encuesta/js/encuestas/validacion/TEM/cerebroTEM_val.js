function validarEncuesta() {
    ...
    if ($("input[name='p16']:checked").val() != undefined) {
        if ($("input[name='p16']:checked").val() == 1) {
            if ($("#p16_1").val() == "") {
                $("#p16_1").parent().parent().parent().append("<p class='MessageValidation'>Seleccione el motivo</p>");
                _IsValid = false;
                _msj += "Pregunta 16: Seleccione el motivo\n";
            }
        }
    } else {
        $("#p16_1").parent().parent().parent().append("<p class='MessageValidation'>Seleccione el contraste</p>");
        _IsValid = false;
        _msj += "Pregunta 16: Seleccione el contraste\n";
    }
