function validarEncuesta() {
    ...
    if ($("input[name='mol_p1']:checked").val() != undefined) {
        if ($("input[name='mol_p1']:checked").val() == "true") {
            if ($("#rpta_p1 input:checkbox:checked").length == 0) {
                ...
                //otros
                if ($("#mol_p1_8").is(":checked") && $("#mol_p1_8_1").val().trim() == "") {
                    $("#mol_p1_8").parent().parent().append("<p class='MessageValidation'>Debe especificar</p>");
                    _IsValid = false;
    ...
    if ($("input[name='mol_p2']:checked").val() != undefined) {
        if ($("input[name='mol_p2']:checked").val() == "true") {
            if ($("#rpta_p2 input:checkbox:checked").length == 0) {
                ...
                if ($("#mol_p2_5").is(":checked") && $("#mol_p2_5_1").val().trim() == "") {
                    $("#mol_p2_5").parent().parent().append("<p class='MessageValidation'>Debe ingresar la ubicación</p>");
                    _IsValid = false;
                    _msj += "Pregunta 2: Ingresar respuesta\n";
                }
                //otros
                if ($("#mol_p2_6").is(":checked") && $("#mol_p2_6_1").val().trim() == "") {
                    $("#mol_p2_6").parent().parent().append("<p class='MessageValidation'>Debe especificar</p>");
                    _IsValid = false;
                    _msj += "Pregunta 2: Ingresar respuesta\n";
                }
            }
    ...
    //3
    if ($("#mol_p3_2").is(":checked") && $("#mol_p3_2_1").val().trim() == "") {
        $("#mol_p3_2").parent().parent().append("<p class='MessageValidation'>Debe especificar</p>");
        _IsValid = false;
