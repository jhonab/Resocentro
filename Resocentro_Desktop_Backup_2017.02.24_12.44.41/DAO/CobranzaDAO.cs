...
{2}
 ...
 group by  CONVERT(CHAR(5),a.fechayhora,108) ,a.numeroatencion,(p.apellidos+' '+p.nombres) ,a.numerocita,p.codigopaciente";
                                            ...
                                            if (readerCoa.HasRows)
                                            {
                                                while (readerCoa.Read())
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        throw new Exception("La carta registrada en el Sistema no esta correctamente(SAIR), verifique los datos y vuelva a guardarla.");
                                                    }
                                                }
                                            }
                                            else
        ...
        public List<EmpresaFacturacion> buscarEmpresas(string ruc)
        {
            ...
            string queryString = "select a.ruc,isnull(cs.descripcion,'-') aseguradora,a.razonsocial,a.domiciliofiscal,isnull(cs.codigocompaniaseguro,-1)codigocompaniaseguro from ASEGURADORA a left join  COMPANIASEGURO cs  on cs.ruc=a.ruc where a.razonsocial like '%{0}%' or a.ruc ='{0}'";
            using (DATABASEGENERALEntities db = new DATABASEGENERALEntities())
            {
        ...
        private string getConexionOracle(int empresa)
        {
            if (empresa == 1)
        ...
        public ERROR_ANULACION_DOCUMENTO validarAnulacionDocumento(string numeroDocumento, string tipodocumento, string empresasucursal, out DateTime fecharegistro)
        {
            ...
            string sqlTVVFACUTRA = "select estado,fechaemitio from documento where numerodocumento='" + numeroDocumento + "' and tipodocumento='" + tipodocumento + "' and codigounidad=" + empresasucursal.Substring(0, 1) + ";";
            using (DATABASEGENERALEntities db = new DATABASEGENERALEntities())
            {
                using (SqlConnection connection = new SqlConnection(db.Database.Connection.ConnectionString))
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            while (reader.Read())
                            {
                                ...
                                if (fecharegistro.Month != Tool.getDatetime().Month)
                                    result = ERROR_ANULACION_DOCUMENTO.DOCUMENTO_RESTRINGIDO;
                            }
                        }
        ...
        public bool anularDocumento(string numeroDocumento, string tipodocumento, string empresasucursal, string motivo, MySession session)
        {
            ...
            using (DATABASEGENERALEntities db = new DATABASEGENERALEntities())
            {
                using (SqlConnection connection = new SqlConnection(db.Database.Connection.ConnectionString))
                {
                    ...
                    connection.Open();
                    SqlTransaction transaction = connection.BeginTransaction("GuardarDocumento");
                    string sqlDocumento = "UPDATE DOCUMENTO SET estado='A',subtotal='0.0',igv='0.0',total='0.0',porconcepto='Por:', fecha_cancelacion=getdate(),usu_cancela='" + session.codigousuario + "',motivos_adicionales='" + motivo + "' WHERE tipodocumento='" + tipodocumento + "' AND numerodocumento='" + numeroDocumento + "'  AND codigounidad='" + empresasucursal.Substring(0, 1) + "';";
        ...
        public void cambiarestadoPagado(DocumentoSunat documento)
        {
            using (DATABASEGENERALEntities db = new DATABASEGENERALEntities())
            {
                string sqlUpdate = "UPDATE EXAMENXATENCION SET estadoestudio='P' WHERE numeroatencion='" + documento.numeroatencion + "' AND estadoestudio='" + (documento.empresa == 2 ? "A" : "R") + "';";
        ...
        public void procesarEstudios(string numerocita, string codigopaciente, string numeroatencion, int empresa)
        {
            using (DATABASEGENERALEntities db = new DATABASEGENERALEntities())
            {
                ...
                if (empresa == 1)
                    sqlUpdateAtencion = "UPDATE EXAMENXATENCION SET estadoestudio='P' WHERE numeroatencion='" + numeroatencion + "' AND estadoestudio='R';";
                else
                    sqlUpdateAtencion = "UPDATE EXAMENXATENCION SET estadoestudio='P' WHERE numeroatencion='" + numeroatencion + "' AND estadoestudio='A';";
        ...
        public List<FacturacionEntity> getListaFacturacion(string empresa, string fecha)
        {
            using (DATABASEGENERALEntities db = new DATABASEGENERALEntities())
            {
                List<FacturacionEntity> lista = new List<FacturacionEntity>();
                string sqlString = "exec getListaFacturacion '{0}','{1}'";
                using (SqlConnection connection = new SqlConnection(db.Database.Connection.ConnectionString))
                {
                    using (SqlCommand command = new SqlCommand(string.Format(sqlString, empresa, fecha), connection))
                    {
                        connection.Open();
                        SqlDataReader reader = command.ExecuteReader();
                        try
...
<tr style=""padding:0;"">
   ...
   <td style=""text-align:right;font-weight:bold;padding:0;""><span class=""font_sizesmall"">{1}</span></td>
               ...
               /* else
                ...
                var html = @"<!DOCTYPE html><html lang=""es-pe"" xmlns=""http://www.w3.org/1999/xhtml""><head> <title></title></head><body><table style=""width:100%""> <tr> <td style=""width:70%;""> <p class=""logo""><img src=""http://extranet.resocentro.com:5050/PaginaWeb/DocElectronico/{17}.png""/></p><p class=""font_sizesmall""> {18} <br/>{0}<br/></p></td><td class=""tr-tipodocumento"" colspan=""2""> <p style=""""> <span class=""font_sizemedium"">RUC {19}</span><br/> <span class=""font_sizebig""> <b style=""margin-top:20px!important;"">{4}</b> </span><br/> <span class=""font_sizemedium"">{5}</span> </p></td></tr><tr> <td style=""width:70%;""> <p class=""font_sizesmall ""> <br/><b>ADQUIRIENTE</b> <br/>{1}<br/>{2}<br/>{3}<br/>{14}</p></td><td style=""vertical-align:top;"" colspan=""2""> <p class=""font_sizesmall""> FECHA EMISIÓN: {6}<br/>MONEDA: {7}<br/>IGV {8}</p></td></tr><tr> <td colspan=""3""> <table class=""tabla_item tabla"" style=""margin-top:30px;""> <tr> <th class=""tr_cantidad""><b class=""font_sizemedium"">CANT.</b></th> <th class=""tr_descripcion""><b class=""font_sizemedium"">DESCRIPCIÓN</b></th> <th class=""tr_unitario""><b class=""font_sizemedium derecha"">P. UNIT.</b></th> <th class=""tr_descuento""><b class=""font_sizemedium derecha"">PROM.</b></th> <th class=""tr_descuento""><b class=""font_sizemedium derecha"">{13}</b></th><th class=""tr_descuento""><b class=""font_sizemedium derecha"">DESC.</b></th> <th class=""tr_importe""><b class=""font_sizemedium derecha"">IMPORTE</b></th> </tr>{9}</table> </td></tr><tr> <td></td><td colspan=""2""> <table class=""tabla"">{10}</table> </td></tr><tr> <td colspan=""3"" class="""" style=""text-align:center;text-transform:uppercase;""> <b class=""font_sizebig"">{15}</b><br/><p class=""font_sizesmall separacion-media""><br/><br/><b>IMPORTE EN LETRAS {11}</b><br/>{20}</p></td></tr><tr> <td colspan=""3"" class="""" style=""text-align:center;""> <p class=""font_sizesmall separacion-media""> <br/><br/><b>Representación impresa del Comprobante de Venta Electrónica, para consultar el documento visita www.resocentro.com<br/>Autorizado mediante Resolucion de Intendencia No. {16}</b> <br/> <img class=""img-pdf"" src=""{12}""/> </p></td></tr></table></body></html>";
                ...
                html = string.Format(html,
                    ...
                    pathCodeBar,//12
                    cabeceraSeguro,//13
                    "<br/> <b>DATOS ADICIONALES</b><br/>" + (documento.textoinformacion.ToUpper()).Replace("\n", "<br/>") + (documento.tipoDocumento == (int)TIPO_DOCUMENTO_ELECTRONICO.NOTA_DE_CREDITO || documento.tipoDocumento == (int)TIPO_DOCUMENTO_ELECTRONICO.NOTA_DE_DEBITO ? "DOCUMENTO REFERENCIA:" + documento.numeroDocumentoReferencia : ""), //14
