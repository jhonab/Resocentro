...
namespace Resocentro_Desktop.DAO
{
    public class Tool
    {
        ...
        public int[] CompaniasNOAfectasTEM{get;set;}
        public Tool()
        {
            //DECIMALES
            ...
            using (DATABASEGENERALEntities db = new DATABASEGENERALEntities())
            {
                using (SqlConnection connection = new SqlConnection(db.Database.Connection.ConnectionString))
                {
                    string query = "select valor from table_config where id=1";
                    SqlCommand command = new SqlCommand(query, connection);
                    connection.Open();
                    SqlDataReader reader = command.ExecuteReader();
                    try
                    {
                        while (reader.Read())
                        {
                            CompaniasNOAfectasTEM=Array.ConvertAll(((reader["valor"]).ToString()).Split(','), s => int.Parse(s));
                        }
                    }
                    finally
                    {
        ...
        public static string UrlDocumentosAdjuntos { get { return @"http://192.168.0.5:5002/"; } }
